...
class ChatUiState(messages: List<ChatMessage> = emptyList()) : UiState {
    ...
    override fun appendMessage(id: String, text: String, done: Boolean) {
        ...
        if (index != -1) {
            ...
            _messages[index] = _messages[index].copy(message = newText, isLoading = !done)
        }
    }

    override fun addMessage(text: String, author: String): String {
        val chatMessage = ChatMessage(message = text, author = author)
        _messages.add(chatMessage)
        return chatMessage.id
    }
}
 ...
 */
class GemmaUiState(messages: List<ChatMessage> = emptyList()) : UiState {
    ...
    override fun addMessage(text: String, author: String): String {
        ...
        _messages.add(chatMessage)
        Log.d("MainViewModel", "Message added: ${chatMessage.message}")
        return chatMessage.id
    }
}